...
def main(access_key_ss, secret_key_ss, param_file_ss):
    ...
    cfn_client_ss = boto3.client('cloudformation', region_ss, aws_access_key_id=access_key_ss,
                                 aws_secret_access_key=secret_key_ss)
    ...
    result_ss = cfn_client_ss.create_stack(StackName=cfn_stack_name_ss, DisableRollback=True,
                                           TemplateURL=template_url_ss, Parameters=fetch_stack_parameters_ss,
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        ...
        access_key_ss = os.environ['AWS_ACCESS_KEY_ID']
        secret_key_ss = os.environ['AWS_SECRET_ACCESS_KEY']
        param_file_ss = os.environ['PARAMETER_FILE']
